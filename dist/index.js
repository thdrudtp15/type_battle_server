...
const getRoomPlayer = (roomId, socketId) => {
    if (!roomId || !socketId)
        return null;
...
const getPlayers = (room, socketId) => {
    if (!room)
        return;
    let playerKey = null;
    if (room.players.player1.socketId === socketId) {
...
io.on('connection', (socket) => {
                    ...
                    newRoom.remainingTime = setInterval(() => {
                        ...
                        if (remainingTime <= 0) {
                            if (newRoom.matchStartTime) {
                                ...
                                if (!player.finished)
                                    player.finished = finishedTime;
                                if (!opponent.finished)
                                    opponent.finished = finishedTime;
                            }
    ...
    socket.on('match_log', (roomId, log) => {
        const room = matchRooms.get(roomId);
        if (!room)
            return;
    ...
    socket.on('match_cpm', (roomId, cpm) => {
